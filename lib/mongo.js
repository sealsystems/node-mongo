...
const faultTolerantConnect = async function (connectionString, options) {
  ...
  const dbObjects = await retry(
    async () => {
      ...
      delete mongoOptions.bucketName;

      const client = new mongodb.MongoClient(connectionString, mongoOptions);
...
mongo.db = async function (connectionString, options = {}) {
  ...
  db.getMongoClient = function () {
    return client;
  };

  db.getMongoModule = function () {
    return mongodb;
  };
  ...
  db.gridfs = function () {
    if (!db.gridfs.cachedfs) {
      db.gridfs.cachedfs = {
        bucket: new mongodb.GridFSBucket(db, { bucketName }),
        ...
        getChunksCollection() {
          return db.collection(`${bucketName}.chunks`);
        },
